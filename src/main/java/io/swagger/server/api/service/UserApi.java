package io.swagger.server.api.service;

import io.swagger.server.api.model.InlineResponse2001;
import io.swagger.server.api.model.InlineResponse2002;
import io.swagger.server.api.model.InlineResponse2003;
import io.swagger.server.api.model.InlineResponse400;

import io.vertx.core.Handler;
import io.vertx.core.AsyncResult;
import io.vertx.ext.web.validation.RequestParameter;
import io.vertx.ext.web.api.service.ServiceRequest;
import io.vertx.ext.web.api.service.ServiceResponse;
import io.vertx.ext.web.api.service.WebApiServiceGen;

import java.util.List;
import java.util.Map;

@WebApiServiceGen
public interface UserApi  {

    String WEBSERVICE_ADDRESS_USERAPI = "";
    String OPERATION_ID_DELETEUSER = "deleteUser";
    String OPERATION_ID_GETUSERID = "getUserId";
    String OPERATION_ID_SETUSER = "setUser";
    String OPERATION_ID_UPDATEUSER = "updateUser";

    void deleteUser(RequestParameter body, ServiceRequest request, Handler<AsyncResult<ServiceResponse>> resultHandler);
    void getUserId(RequestParameter body, ServiceRequest request, Handler<AsyncResult<ServiceResponse>> resultHandler);
    void setUser(RequestParameter body, ServiceRequest request, Handler<AsyncResult<ServiceResponse>> resultHandler);
    void updateUser(RequestParameter body, ServiceRequest request, Handler<AsyncResult<ServiceResponse>> resultHandler);
}
